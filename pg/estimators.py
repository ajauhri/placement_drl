# Based out of -- https://github.com/dennybritz/reinforcement-learning/tree/master/PolicyGradient/a3c
...
class ActorEstimator:
    def __init__(self, x, y, num_outputs, reuse=False, trainable=True):
        self.num_outputs = num_outputs
        
        # Our input is a RGB frame of shape 160, 160 each
        self.states = tf.placeholder(shape=[None, x, y, 3], 
        ...
        with tf.variable_scope("actor_net"):
            self.logits = tf.contrib.layers.fully_connected(fc1, 
...
class CriticEstimator():
    def __init__(self, x, y, reuse=False, trainable=True):
        ...
        with tf.variable_scope("critic_net"):
            ...
            if trainable:
                ...
                self.grads_and_vars = [[grad, var] for grad, 
                        var in self.grads_and_vars if grad is not None]
                self.train_op = self.optimizer.apply_gradients(
                        self.grads_and_vars,
                        global_step=tf.contrib.framework.get_global_step())
            ...
            sumaries = [s for s in summary_ops if\
                    "policy_net" in s.name or "shared" in s.name]
